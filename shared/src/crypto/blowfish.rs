...
impl Blowfish {
    ...
    pub fn encrypt(&self, mut xl: u32, mut xr: u32) -> (u32, u32) {
        xl ^= self.p[0];  xr ^= self.f(xl);
        xr ^= self.p[1];  xl ^= self.f(xr);
        xl ^= self.p[2];  xr ^= self.f(xl);
        xr ^= self.p[3];  xl ^= self.f(xr);
        xl ^= self.p[4];  xr ^= self.f(xl);
        xr ^= self.p[5];  xl ^= self.f(xr);
        xl ^= self.p[6];  xr ^= self.f(xl);
        xr ^= self.p[7];  xl ^= self.f(xr);
        xl ^= self.p[8];  xr ^= self.f(xl);
        xr ^= self.p[9];  xl ^= self.f(xr);
        xl ^= self.p[10]; xr ^= self.f(xl);
        xr ^= self.p[11]; xl ^= self.f(xr);
        xl ^= self.p[12]; xr ^= self.f(xl);
        xr ^= self.p[13]; xl ^= self.f(xr);
        xl ^= self.p[14]; xr ^= self.f(xl);
        xr ^= self.p[15]; xl ^= self.f(xr);
    ...
    /// Odszyfrowanie jednego bloku 64 bit = tuple (u32,u32).
